...
function createButtons(ctx: Ctx, onRequestLink?: () => void): ToolbarButton[] {
  ...
  return [
    { title: '加粗', label: 'B', cls: 'is-bold', run: () => manager().call(toggleStrongCommand.key) },
    { title: '斜体', label: 'I', cls: 'is-italic', run: () => manager().call(toggleEmphasisCommand.key) },
    {
      title: '删除线',
      label: 'S',
      cls: 'is-strike',
    ...
    { title: '链接', label: '🔗', cls: 'is-link', run: () => onRequestLink?.() }
  ]
}
 ...
 */
export const selectionToolbar = (options: SelectionToolbarOptions = {}) => $prose((ctx) => {
